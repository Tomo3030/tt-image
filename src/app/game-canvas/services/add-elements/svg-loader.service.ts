...
export class SvgLoaderService {
  ...
  async placeSvgsOnCanvas(
    ...
    if (svgPlacements['asset-container'])
  ...
  private loadSvgsToDropzones(
    ...
    Object.entries(svgPlacement).forEach(([key, path]) => {
      if (key !== 'asset-container') {
        ...
        //can set the dz scale here because in some cases the each dz has a different scale
        if (!dz) return;
  ...
  private getAssetGrid(assetContainer: any, assetLength: number) {
    ...
    for (let row = 0; row < rows; row++) {
      ...
      for (let col = 0; col < itemsPerRow; col++) {
        ...
        let left = itemWidth * col + offsetLeft + 25; // 25 is magic -- it comes from the padding in the asset container
